          ...
          <div class="pop-new-card__categories categories">
            ...
            <div class="categories__themes">
              <div
                ...
                :key="category.name"
                :class="['categories__theme', getThemeClass(category.name), { '_active-category': formData.topic === category.name }]"
                @click="formData.topic = category.name"
              >
                <p :class="getThemeClass(category.name)">{{ category.name }}</p>
              </div>
            </div>
          </div>
          <button class="form-new__create _hover01" id="btnCreate" @click.prevent="createCard">Создать задачу</button>
        </div>
      </div>
    </div>
  </div>
